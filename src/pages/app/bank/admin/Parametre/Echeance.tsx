...
const Echeance = () => {
    ...
    const onEdit = (data: any) => {
        ...
        setErrors(writeErrors({ ...errors }, {}));
        setIdBanque(data.id);
        setTitle('Edition de la échéance');
        setChoice(menu.edit);
    }
        ...
        <Layout title="Les Echéances">
            ...
            {isLoading == false && <div className="row">
                {(menu.add == choice || menu.edit == choice) && <div className="col-md-8 col-lg-5">
                    <Wrapper title={title}>
                        <div className="text-right"><button type="button" onClick={() => onChoice(menu.list)} className="btn btn-link">Les Echéances</button></div>
                        <form onSubmit={onAddOrEdit}>
                            <Input label='Intitule Echéance' report={errors.libelle} name='libelle' data={banqueAdd} update={setbanqueAdd} required />
                            <div className="mt-2">
                ...
                {menu.list == choice && <div className="col-6">
                    <Wrapper title="Liste des Modes Paiements">
                        <div className="text-right mb-2">
                            <button type="button" onClick={() => onChoice(menu.add)} className="btn btn-primary text-right">Ajouter une échéance</button>
                        </div>
                        <div className="table-responsive">
                            <table className="table">
                                <thead>
                                    <tr>
                                        <th>Intitule De l'écheance</th>
                                        <th className="text-right">Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {echeances?.content?.map((echeance: any) => {

                                        return (
                                            <tr key={echeance.id}>
                                                <td>{echeance.libelle}</td>
                                                <td className="text-right">
                                                    <Link to={"#"} onClick={() => onEdit(echeance)} className="text-success"><i className="fa fa-eyedropper"></i> </Link>
                                                    <Link to={"#"} onClick={() => onRemove(echeance.id)} className="text-danger"><i className="fa fa-trash"></i> </Link>
                                                </td>
                                            </tr>
                                        )
                                    })}
                        ...
                        <Pagination update={setPage} paginator={echeances} />
                    </Wrapper>
                </div>}
            </div>}
        </Layout>
