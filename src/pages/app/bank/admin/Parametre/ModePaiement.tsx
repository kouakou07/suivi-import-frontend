...
const ModePaiement = () => {
    ...
    const [modepaiements, setModePaiements] = useState<any>({});
    ...
    const onChoice = (target: number) => {
        if (target == menu.add) {
            ...
            setTitle('Ajout de Mode Paiement');
            setIdBanque(0);
    ...
    const onEdit = (data: any) => {
        ...
        setErrors(writeErrors({ ...errors }, {}));
        setIdBanque(data.id);
        setTitle('Edition de mode paiement');
        setChoice(menu.edit);
    }
    ...
    return (
        <Layout title="Les Modes Paiements">
            ...
            {isLoading == false && <div className="row">
                {(menu.add == choice || menu.edit == choice) && <div className="col-md-8 col-lg-5">
                    <Wrapper title={title}>
                        <div className="text-right"><button type="button" onClick={() => onChoice(menu.list)} className="btn btn-link">Les Modes de paiements</button></div>
                        <form onSubmit={onAddOrEdit}>
                            <Input label='Intitule Mode Paiement' report={errors.libelle} name='libelle' data={banqueAdd} update={setbanqueAdd} required />
                            <div className="mt-2">
                ...
                {menu.list == choice && <div className="col-6">
                    <Wrapper title="Liste des Modes Paiements">
                        <div className="text-right mb-2">
                            <button type="button" onClick={() => onChoice(menu.add)} className="btn btn-primary text-right">Ajouter un Mode de Paiement</button>
                        </div>
                        <div className="table-responsive">
                            <table className="table">
                                <thead>
                                    <tr>
                                        <th>Intitule Du Mode Paiement</th>
                                        <th className="text-right">Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {modepaiements?.content?.map((modepaiement: any) => {

                                        return (
                                            <tr key={modepaiement.id}>
                                                <td>{modepaiement.libelle}</td>
                                                <td className="text-right">
                                                    <Link to={"#"} onClick={() => onEdit(modepaiement)} className="text-success"><i className="fa fa-eyedropper"></i> </Link>
                                                    <Link to={"#"} onClick={() => onRemove(modepaiement.id)} className="text-danger"><i className="fa fa-trash"></i> </Link>
                                                </td>
                                            </tr>
                                        )
                                    })}
                        ...
                        <Pagination update={setPage} paginator={modepaiements} />
                    </Wrapper>
                </div>}
            </div>}
        </Layout>
