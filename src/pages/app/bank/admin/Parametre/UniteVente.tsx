...
const UniteVente = () => {
    ...
    const [uniteVentes, setUniteVentes] = useState<any>({});
    ...
    const onEdit = (data: any) => {
        ...
        setErrors(writeErrors({ ...errors }, {}));
        setIdBanque(data.id);
        setTitle('Edition de Unite Vente');
        setChoice(menu.edit);
    }
    ...
    return (
        <Layout title="Les des unites ventes">
            ...
            {isLoading == false && <div className="row">
                {(menu.add == choice || menu.edit == choice) && <div className="col-md-8 col-lg-5">
                    <Wrapper title={title}>
                        <div className="text-right"><button type="button" onClick={() => onChoice(menu.list)} className="btn btn-link">Les Unites de Ventes</button></div>
                        <form onSubmit={onAddOrEdit}>
                            <Input label='Intitule Unite Ventes' report={errors.libelle} name='libelle' data={banqueAdd} update={setbanqueAdd} required />
                            <div className="mt-2">
                ...
                {menu.list == choice && <div className="col-6">
                    <Wrapper title="Liste des Unites Ventes">
                        <div className="text-right mb-2">
                            <button type="button" onClick={() => onChoice(menu.add)} className="btn btn-primary text-right">Ajouter une Unite de Vente</button>
                        </div>
                        <div className="table-responsive">
                            <table className="table">
                                <thead>
                                    <tr>
                                        <th>Intitule Unite Vente</th>
                                        <th className="text-right">Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {uniteVentes?.content?.map((uniteVente: any) => {

                                        return (
                                            <tr key={uniteVente.id}>
                                                <td>{uniteVente.libelle}</td>
                                                <td className="text-right">
                                                    <Link to={"#"} onClick={() => onEdit(uniteVente)} className="text-success"><i className="fa fa-eyedropper"></i> </Link>
                                                    <Link to={"#"} onClick={() => onRemove(uniteVente.id)} className="text-danger"><i className="fa fa-trash"></i> </Link>
                                                </td>
                                            </tr>
                                        )
                                    })}
                        ...
                        <Pagination update={setPage} paginator={uniteVentes} />
                    </Wrapper>
                </div>}
            </div>}
        </Layout>
